# Epics

_Auto-generated by `housekeep.py`. Do not edit manually._

**Overall:** 🔵 **active** — ████████░░ 48/58 (83%) across 9 groups — 2 open · 5 active · 3 paused · 48 closed

## Index

| Epic | Title | Status | Open | Active | Paused | Closed | Done |
|------|-------|--------|-----:|-------:|-------:|-------:|------|
| [EPIC-001](#epic-001-align-with-circuitsmith-framework) | Align with CircuitSmith framework | 🟢 closed | 0 | 0 | 0 | 13 | ██████████ 100% |
| [EPIC-002](#epic-002-markdown-inventory-schema) | Markdown inventory schema | 🔵 **active** | 0 | 0 | 1 | 4 | ████████░░ 80% |
| [EPIC-003](#epic-003-skill-path--inventory-add-and-inventory-page) | Skill path — /inventory-add and /inventory-page | 🟢 closed | 0 | 0 | 0 | 3 | ██████████ 100% |
| [EPIC-004](#epic-004-project-setup--port-from-circuitsmith) | Project setup — port from CircuitSmith | 🟢 closed | 0 | 0 | 0 | 10 | ██████████ 100% |
| [EPIC-005](#epic-005-usb-camera-capture) | USB camera capture | 🔵 **active** | 0 | 2 | 0 | 5 | ███████░░░ 71% |
| [EPIC-006](#epic-006-visual-recognition--dinov2--vlm) | Visual recognition — DINOv2 + VLM | 🔵 **active** | 0 | 1 | 0 | 5 | ████████░░ 83% |
| [EPIC-007](#epic-007-metadata-enrichment--nexar) | Metadata enrichment — Nexar | 🟢 closed | 0 | 0 | 0 | 6 | ██████████ 100% |
| [EPIC-008](#epic-008-resistor-color-band-reader) | Resistor color-band reader | 🔵 **active** | 2 | 2 | 0 | 2 | ███░░░░░░░ 33% |
| [EPIC-009](#epic-009-idea-012-integration-pass-follow-ups) | IDEA-012 integration-pass follow-ups | 🔵 **active** | 0 | 0 | 2 | 0 | ░░░░░░░░░░ 0% |

---

## EPIC-001: Align with CircuitSmith framework

[↑ back to top](#index)

**Status:** 🟢 closed — ██████████ 13/13 (100%)

```mermaid
graph LR
    TASK_001["TASK-001"]:::closed
    TASK_002["TASK-002"]:::closed
    TASK_003["TASK-003"]:::closed
    TASK_004["TASK-004"]:::closed
    TASK_005["TASK-005"]:::closed
    TASK_006["TASK-006"]:::closed
    TASK_007["TASK-007"]:::closed
    TASK_008["TASK-008"]:::closed
    TASK_009["TASK-009"]:::closed
    TASK_010["TASK-010"]:::closed
    TASK_011["TASK-011"]:::closed
    TASK_012["TASK-012"]:::closed
    TASK_013["TASK-013"]:::closed
    TASK_001 --> TASK_002
    TASK_002 --> TASK_003
    TASK_003 --> TASK_004
    TASK_004 --> TASK_005
    TASK_005 --> TASK_008
    TASK_005 --> TASK_009
    TASK_009 --> TASK_010
    TASK_001 --> TASK_011
    TASK_002 --> TASK_011
    TASK_003 --> TASK_011
    TASK_004 --> TASK_011
    TASK_005 --> TASK_011
    TASK_006 --> TASK_012
    TASK_002 --> TASK_013
    TASK_006 --> TASK_013
    TASK_001 ~~~ TASK_006 ~~~ TASK_007
    click TASK_001 "closed/task-001-delete-sync-layer.md"
    click TASK_002 "closed/task-002-python-skeleton.md"
    click TASK_003 "closed/task-003-replace-pre-commit.md"
    click TASK_004 "closed/task-004-upgrade-commit-skill.md"
    click TASK_005 "closed/task-005-settings-json.md"
    click TASK_006 "closed/task-006-docs-verbatim-port.md"
    click TASK_007 "closed/task-007-architecture-doc.md"
    click TASK_008 "closed/task-008-security-review-hooks.md"
    click TASK_009 "closed/task-009-codeowner-mechanism.md"
    click TASK_010 "closed/task-010-codeowner-starter-skills.md"
    click TASK_011 "closed/task-011-claude-md-rewrite.md"
    click TASK_012 "closed/task-012-epic-run-and-autonomy.md"
    click TASK_013 "closed/task-013-apply-branch-protection.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 1 | ~~[TASK-001](closed/task-001-delete-sync-layer.md)~~ | ~~Delete awesome-task-system/ and scripts/sync_task_system.py~~ | 🟢 closed | Small |
| 2 | ~~[TASK-002](closed/task-002-python-skeleton.md)~~ | ~~Author Python project skeleton (pyproject, requirements-dev, CI, conftest, gitignore)~~ | 🟢 closed | Small |
| 3 | ~~[TASK-003](closed/task-003-replace-pre-commit.md)~~ | ~~Replace scripts/pre-commit with the CircuitSmith version~~ | 🟢 closed | Small |
| 4 | ~~[TASK-004](closed/task-004-upgrade-commit-skill.md)~~ | ~~Upgrade /commit skill and commit-pathspec.sh to the CircuitSmith versions~~ | 🟢 closed | Small |
| 5 | ~~[TASK-005](closed/task-005-settings-json.md)~~ | ~~Author .claude/settings.json with full allowlist + deny~~ | 🟢 closed | Small |
| 6 | ~~[TASK-006](closed/task-006-docs-verbatim-port.md)~~ | ~~Port the 13 verbatim developer docs from CircuitSmith~~ | 🟢 closed | Medium |
| 7 | ~~[TASK-007](closed/task-007-architecture-doc.md)~~ | ~~Write docs/developers/ARCHITECTURE.md for the PartsLedger pipeline~~ | 🟢 closed | Medium |
| 8 | ~~[TASK-008](closed/task-008-security-review-hooks.md)~~ | ~~Port security-review hooks (pre-merge-commit, post-merge, pre-rebase)~~ | 🟢 closed | Medium |
| 9 | ~~[TASK-009](closed/task-009-codeowner-mechanism.md)~~ | ~~Port codeowner reminder mechanism (hook + registry + PreToolUse)~~ | 🟢 closed | Small |
| 10 | ~~[TASK-010](closed/task-010-codeowner-starter-skills.md)~~ | ~~Author starter co-* skills capturing PartsLedger invariants~~ | 🟢 closed | Small |
| 11 | ~~[TASK-011](closed/task-011-claude-md-rewrite.md)~~ | ~~Rewrite CLAUDE.md to mirror CircuitSmith's verbatim~~ | 🟢 closed | Small |
| 12 | ~~[TASK-012](closed/task-012-epic-run-and-autonomy.md)~~ | ~~Port /epic-run skill and AUTONOMY.md, sweep HIL frontmatter on open tasks~~ | 🟢 closed | Medium |
| 13 | ~~[TASK-013](closed/task-013-apply-branch-protection.md)~~ | ~~Apply server-side branch protection to tgd1975/PartsLedger main~~ | 🟢 closed | Small |

## EPIC-002: Markdown inventory schema

[↑ back to top](#index)

**Status:** 🔵 **active** — ████████░░ 4/5 (80%)

```mermaid
graph LR
    TASK_018["TASK-018"]:::paused
    TASK_014["TASK-014"]:::closed
    TASK_015["TASK-015"]:::closed
    TASK_016["TASK-016"]:::closed
    TASK_017["TASK-017"]:::closed
    TASK_022["TASK-022"]:::closedExt
    TASK_014 --> TASK_018
    TASK_014 --> TASK_015
    TASK_014 --> TASK_016
    TASK_022 --> TASK_016
    TASK_014 --> TASK_017
    TASK_022 --> TASK_017
    click TASK_018 "open/task-018-inventory-split-support.md"
    click TASK_014 "closed/task-014-source-column-and-section-flex.md"
    click TASK_015 "closed/task-015-parts-page-template-adaptivity.md"
    click TASK_016 "closed/task-016-inventory-writer-module.md"
    click TASK_017 "closed/task-017-inventory-lint-module.md"
    click TASK_022 "closed/task-022-adopt-src-layout.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 5 | [TASK-018](paused/task-018-inventory-split-support.md) | Multi-file INVENTORY.md split support + suggestion trigger | 🟡 **paused** | Large (8-24h) |
| 1 | ~~[TASK-014](closed/task-014-source-column-and-section-flex.md)~~ | ~~Add Source column and maker-choice section taxonomy to INVENTORY.md~~ | 🟢 closed | Medium (2-8h) |
| 2 | ~~[TASK-015](closed/task-015-parts-page-template-adaptivity.md)~~ | ~~Teach /inventory-page to produce part-class-appropriate sections~~ | 🟢 closed | Small (&lt;2h) |
| 3 | ~~[TASK-016](closed/task-016-inventory-writer-module.md)~~ | ~~Implement src/partsledger/inventory/writer.py with upsert_row() contract~~ | 🟢 closed | Large (8-24h) |
| 4 | ~~[TASK-017](closed/task-017-inventory-lint-module.md)~~ | ~~Implement src/partsledger/inventory/lint.py + scripts/lint_inventory.py shim~~ | 🟢 closed | Medium (2-8h) |

## EPIC-003: Skill path — /inventory-add and /inventory-page

[↑ back to top](#index)

**Status:** 🟢 closed — ██████████ 3/3 (100%)

```mermaid
graph TD
    TASK_019["TASK-019"]:::closed
    TASK_020["TASK-020"]:::closed
    TASK_021["TASK-021"]:::closed
    TASK_050["TASK-050"]:::closedExt
    TASK_050 --> TASK_020
    TASK_019 ~~~ TASK_021
    click TASK_019 "closed/task-019-hedge-language-lint.md"
    click TASK_020 "closed/task-020-page-gen-auto-trigger.md"
    click TASK_021 "closed/task-021-family-page-proactive-suggestion.md"
    click TASK_050 "closed/task-050-skill-sync-chain.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 1 | ~~[TASK-019](closed/task-019-hedge-language-lint.md)~~ | ~~Hedge-language lint over inventory/parts/*.md + pre-commit hook~~ | 🟢 closed | Medium (2-8h) |
| 2 | ~~[TASK-020](closed/task-020-page-gen-auto-trigger.md)~~ | ~~Auto-trigger /inventory-page on row creation via /inventory-add~~ | 🟢 closed | Medium (2-8h) |
| 3 | ~~[TASK-021](closed/task-021-family-page-proactive-suggestion.md)~~ | ~~Family-page proactive suggestion at add-time + page-gen-time~~ | 🟢 closed | Medium (2-8h) |

## EPIC-004: Project setup — port from CircuitSmith

[↑ back to top](#index)

**Status:** 🟢 closed — ██████████ 10/10 (100%)

```mermaid
graph LR
    TASK_022["TASK-022"]:::closed
    TASK_023["TASK-023"]:::closed
    TASK_024["TASK-024"]:::closed
    TASK_025["TASK-025"]:::closed
    TASK_026["TASK-026"]:::closed
    TASK_027["TASK-027"]:::closed
    TASK_028["TASK-028"]:::closed
    TASK_029["TASK-029"]:::closed
    TASK_030["TASK-030"]:::closed
    TASK_031["TASK-031"]:::closed
    TASK_022 --> TASK_023
    TASK_022 --> TASK_024
    TASK_022 --> TASK_025
    TASK_022 --> TASK_026
    TASK_022 --> TASK_027
    TASK_022 --> TASK_028
    TASK_022 --> TASK_029
    TASK_022 --> TASK_030
    TASK_022 --> TASK_031
    TASK_024 --> TASK_031
    click TASK_022 "closed/task-022-adopt-src-layout.md"
    click TASK_023 "closed/task-023-releasing-docs-and-release-skill.md"
    click TASK_024 "closed/task-024-github-workflows.md"
    click TASK_025 "closed/task-025-uv-lock.md"
    click TASK_026 "closed/task-026-portability-lint.md"
    click TASK_027 "closed/task-027-shim-convention-doc.md"
    click TASK_028 "closed/task-028-scripts-drift-audit.md"
    click TASK_029 "closed/task-029-optional-dependencies-extras.md"
    click TASK_030 "closed/task-030-adr-0001-library-as-installable-package.md"
    click TASK_031 "closed/task-031-bootstrap-readme-quickstart.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 1 | ~~[TASK-022](closed/task-022-adopt-src-layout.md)~~ | ~~Adopt src/partsledger/ layout in pyproject~~ | 🟢 closed | Medium (2-8h) |
| 2 | ~~[TASK-023](closed/task-023-releasing-docs-and-release-skill.md)~~ | ~~Port RELEASING.md and /release skill; rewrite semver for three public surfaces~~ | 🟢 closed | Medium (2-8h) |
| 3 | ~~[TASK-024](closed/task-024-github-workflows.md)~~ | ~~Add .github/workflows/ci.yml and release.yml~~ | 🟢 closed | Medium (2-8h) |
| 4 | ~~[TASK-025](closed/task-025-uv-lock.md)~~ | ~~Adopt uv.lock for reproducible installs~~ | 🟢 closed | Small (&lt;2h) |
| 5 | ~~[TASK-026](closed/task-026-portability-lint.md)~~ | ~~Add src/partsledger/_dev/portability_lint.py with scripts/ shim~~ | 🟢 closed | Medium (2-8h) |
| 6 | ~~[TASK-027](closed/task-027-shim-convention-doc.md)~~ | ~~Document shim convention (scripts/ and skill .py files as thin shims)~~ | 🟢 closed | Small (&lt;2h) |
| 7 | ~~[TASK-028](closed/task-028-scripts-drift-audit.md)~~ | ~~Drift audit on already-copied scripts/ files vs CircuitSmith~~ | 🟢 closed | Medium (2-8h) |
| 8 | ~~[TASK-029](closed/task-029-optional-dependencies-extras.md)~~ | ~~Configure [project.optional-dependencies] for partsledger[resistor-reader] extra~~ | 🟢 closed | Small (&lt;2h) |
| 9 | ~~[TASK-030](closed/task-030-adr-0001-library-as-installable-package.md)~~ | ~~Write ADR-0001 — library as installable package~~ | 🟢 closed | Small (&lt;2h) |
| 10 | ~~[TASK-031](closed/task-031-bootstrap-readme-quickstart.md)~~ | ~~Write README.md / QUICKSTART.md bootstrap section~~ | 🟢 closed | Medium (2-8h) |

## EPIC-005: USB camera capture

[↑ back to top](#index)

**Status:** 🔵 **active** — ███████░░░ 5/7 (71%)

```mermaid
graph LR
    TASK_036["TASK-036"]:::active
    TASK_037["TASK-037"]:::active
    TASK_032["TASK-032"]:::closed
    TASK_033["TASK-033"]:::closed
    TASK_034["TASK-034"]:::closed
    TASK_035["TASK-035"]:::closed
    TASK_038["TASK-038"]:::closed
    TASK_022["TASK-022"]:::closedExt
    TASK_043["TASK-043"]:::activeExt
    TASK_044["TASK-044"]:::closedExt
    TASK_035 --> TASK_036
    TASK_043 --> TASK_036
    TASK_036 --> TASK_037
    TASK_044 --> TASK_037
    TASK_022 --> TASK_032
    TASK_032 --> TASK_033
    TASK_033 --> TASK_034
    TASK_034 --> TASK_035
    TASK_035 --> TASK_038
    TASK_037 --> TASK_038
    click TASK_036 "active/task-036-recognition-overlay-state-machine.md"
    click TASK_037 "active/task-037-secondary-key-dispatch.md"
    click TASK_032 "closed/task-032-camera-selection-wizard.md"
    click TASK_033 "closed/task-033-live-viewfinder-overlays.md"
    click TASK_034 "closed/task-034-capture-trigger-and-still.md"
    click TASK_035 "closed/task-035-camera-cli-wrapper.md"
    click TASK_038 "closed/task-038-capture-slash-skill.md"
    click TASK_022 "closed/task-022-adopt-src-layout.md"
    click TASK_043 "active/task-043-recognition-pipeline-glue.md"
    click TASK_044 "closed/task-044-undo-journal.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 5 | [TASK-036](active/task-036-recognition-overlay-state-machine.md) | Recognition-status overlay state machine + hint-family tokeniser | 🔵 **active** | Large (8-24h) |
| 6 | [TASK-037](active/task-037-secondary-key-dispatch.md) | Secondary key dispatch — R / X / U handlers | 🔵 **active** | Medium (2-8h) |
| 1 | ~~[TASK-032](closed/task-032-camera-selection-wizard.md)~~ | ~~Camera-selection wizard (V4L2 / DirectShow enumeration, friendly names)~~ | 🟢 closed | Large (8-24h) |
| 2 | ~~[TASK-033](closed/task-033-live-viewfinder-overlays.md)~~ | ~~Live viewfinder + capture overlays (framing rect, focus, lighting, trigger hint)~~ | 🟢 closed | Large (8-24h) |
| 3 | ~~[TASK-034](closed/task-034-capture-trigger-and-still.md)~~ | ~~Capture trigger + single-still emit per Output contract~~ | 🟢 closed | Medium (2-8h) |
| 4 | ~~[TASK-035](closed/task-035-camera-cli-wrapper.md)~~ | ~~CLI wrapper python -m partsledger.capture~~ | 🟢 closed | Small (&lt;2h) |
| 7 | ~~[TASK-038](closed/task-038-capture-slash-skill.md)~~ | ~~/capture thin slash-skill subprocess wrapper~~ | 🟢 closed | Small (&lt;2h) |

## EPIC-006: Visual recognition — DINOv2 + VLM

[↑ back to top](#index)

**Status:** 🔵 **active** — ████████░░ 5/6 (83%)

```mermaid
graph LR
    TASK_043["TASK-043"]:::active
    TASK_039["TASK-039"]:::closed
    TASK_040["TASK-040"]:::closed
    TASK_041["TASK-041"]:::closed
    TASK_042["TASK-042"]:::closed
    TASK_044["TASK-044"]:::closed
    TASK_016["TASK-016"]:::closedExt
    TASK_017["TASK-017"]:::closedExt
    TASK_022["TASK-022"]:::closedExt
    TASK_048["TASK-048"]:::closedExt
    TASK_016 --> TASK_043
    TASK_017 --> TASK_043
    TASK_041 --> TASK_043
    TASK_042 --> TASK_043
    TASK_048 --> TASK_043
    TASK_022 --> TASK_039
    TASK_039 --> TASK_040
    TASK_040 --> TASK_041
    TASK_022 --> TASK_042
    TASK_043 --> TASK_044
    click TASK_043 "active/task-043-recognition-pipeline-glue.md"
    click TASK_039 "closed/task-039-recognition-embed-module.md"
    click TASK_040 "closed/task-040-recognition-cache-module.md"
    click TASK_041 "closed/task-041-cache-only-banded-recognition.md"
    click TASK_042 "closed/task-042-vlm-adapter.md"
    click TASK_044 "closed/task-044-undo-journal.md"
    click TASK_016 "closed/task-016-inventory-writer-module.md"
    click TASK_017 "closed/task-017-inventory-lint-module.md"
    click TASK_022 "closed/task-022-adopt-src-layout.md"
    click TASK_048 "closed/task-048-enrichment-orchestrator.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 5 | [TASK-043](active/task-043-recognition-pipeline-glue.md) | Pipeline glue — pipeline.run(image) -&gt; Outcome with re-frame loop and writer hand-off | 🔵 **active** | Large (8-24h) |
| 1 | ~~[TASK-039](closed/task-039-recognition-embed-module.md)~~ | ~~Implement src/partsledger/recognition/embed.py — DINOv2-ViT-S/14 via torch.hub~~ | 🟢 closed | Medium (2-8h) |
| 2 | ~~[TASK-040](closed/task-040-recognition-cache-module.md)~~ | ~~Implement src/partsledger/recognition/cache.py — sqlite-vec backed~~ | 🟢 closed | Medium (2-8h) |
| 3 | ~~[TASK-041](closed/task-041-cache-only-banded-recognition.md)~~ | ~~Cache-only recognition with tight / tight_ambiguous / medium / miss bands~~ | 🟢 closed | Medium (2-8h) |
| 4 | ~~[TASK-042](closed/task-042-vlm-adapter.md)~~ | ~~Implement src/partsledger/recognition/vlm.py — OpenAI-compatible REST adapter~~ | 🟢 closed | Large (8-24h) |
| 6 | ~~[TASK-044](closed/task-044-undo-journal.md)~~ | ~~Undo journal at inventory/.embeddings/undo.toml, depth 1~~ | 🟢 closed | Medium (2-8h) |

## EPIC-007: Metadata enrichment — Nexar

[↑ back to top](#index)

**Status:** 🟢 closed — ██████████ 6/6 (100%)

```mermaid
graph LR
    TASK_045["TASK-045"]:::closed
    TASK_046["TASK-046"]:::closed
    TASK_047["TASK-047"]:::closed
    TASK_048["TASK-048"]:::closed
    TASK_049["TASK-049"]:::closed
    TASK_050["TASK-050"]:::closed
    TASK_016["TASK-016"]:::closedExt
    TASK_022["TASK-022"]:::closedExt
    TASK_043["TASK-043"]:::activeExt
    TASK_022 --> TASK_045
    TASK_045 --> TASK_046
    TASK_022 --> TASK_047
    TASK_016 --> TASK_048
    TASK_045 --> TASK_048
    TASK_046 --> TASK_048
    TASK_047 --> TASK_048
    TASK_043 --> TASK_049
    TASK_048 --> TASK_049
    TASK_048 --> TASK_050
    TASK_049 --> TASK_050
    click TASK_045 "closed/task-045-nexar-graphql-adapter.md"
    click TASK_046 "closed/task-046-nexar-response-cache.md"
    click TASK_047 "closed/task-047-family-datasheet-fallback.md"
    click TASK_048 "closed/task-048-enrichment-orchestrator.md"
    click TASK_049 "closed/task-049-camera-async-dispatch.md"
    click TASK_050 "closed/task-050-skill-sync-chain.md"
    click TASK_016 "closed/task-016-inventory-writer-module.md"
    click TASK_022 "closed/task-022-adopt-src-layout.md"
    click TASK_043 "active/task-043-recognition-pipeline-glue.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 1 | ~~[TASK-045](closed/task-045-nexar-graphql-adapter.md)~~ | ~~Implement src/partsledger/enrichment/nexar.py — OAuth + GraphQL supSearchMpn~~ | 🟢 closed | Medium (2-8h) |
| 2 | ~~[TASK-046](closed/task-046-nexar-response-cache.md)~~ | ~~Implement src/partsledger/enrichment/cache.py — SQLite per-MPN response cache~~ | 🟢 closed | Small (&lt;2h) |
| 3 | ~~[TASK-047](closed/task-047-family-datasheet-fallback.md)~~ | ~~Implement src/partsledger/enrichment/family_datasheets.py — MPN-prefix → URL table~~ | 🟢 closed | Small (&lt;2h) |
| 4 | ~~[TASK-048](closed/task-048-enrichment-orchestrator.md)~~ | ~~Orchestrator enrich(part_id) + writer-integration (no clobber on non-empty cells)~~ | 🟢 closed | Medium (2-8h) |
| 5 | ~~[TASK-049](closed/task-049-camera-async-dispatch.md)~~ | ~~Camera-path async dispatch — dispatch_async() + single-worker thread + enrichment.log~~ | 🟢 closed | Medium (2-8h) |
| 6 | ~~[TASK-050](closed/task-050-skill-sync-chain.md)~~ | ~~Skill-path sync enrichment + page-gen chain (sync for /inventory-add, async for camera)~~ | 🟢 closed | Medium (2-8h) |

## EPIC-008: Resistor color-band reader

[↑ back to top](#index)

**Status:** 🔵 **active** — ███░░░░░░░ 2/6 (33%)

```mermaid
graph LR
    TASK_055["TASK-055"]:::open
    TASK_056["TASK-056"]:::open
    TASK_051["TASK-051"]:::active
    TASK_052["TASK-052"]:::active
    TASK_053["TASK-053"]:::closed
    TASK_054["TASK-054"]:::closed
    TASK_022["TASK-022"]:::closedExt
    TASK_029["TASK-029"]:::closedExt
    TASK_054 --> TASK_055
    TASK_055 --> TASK_056
    TASK_022 --> TASK_051
    TASK_029 --> TASK_051
    TASK_051 --> TASK_052
    TASK_051 --> TASK_053
    TASK_052 --> TASK_054
    TASK_053 --> TASK_054
    click TASK_055 "open/task-055-resistor-trained-detector-v2.md"
    click TASK_056 "open/task-056-resistor-live-overlay-v2.md"
    click TASK_051 "active/task-051-resistor-localisation-v1.md"
    click TASK_052 "active/task-052-resistor-band-reading-eia.md"
    click TASK_053 "closed/task-053-resistor-uniformity-check.md"
    click TASK_054 "closed/task-054-resistor-extra-packaging.md"
    click TASK_022 "closed/task-022-adopt-src-layout.md"
    click TASK_029 "closed/task-029-optional-dependencies-extras.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 5 | [TASK-055](open/task-055-resistor-trained-detector-v2.md) | V2 — small trained detector (YOLO-nano / MobileNet-SSD) for live-view localisation | ⚪ _open_ | Large (8-24h) |
| 6 | [TASK-056](open/task-056-resistor-live-overlay-v2.md) | V2 — live overlay + per-frame stable decoding at ≥10 fps | ⚪ _open_ | Large (8-24h) |
| 1 | [TASK-051](active/task-051-resistor-localisation-v1.md) | V1 — resistor localisation (HSV thresholding + contour finding) on still images | 🔵 **active** | Medium (2-8h) |
| 2 | [TASK-052](active/task-052-resistor-band-reading-eia.md) | V1 — band reading + EIA classifier + orientation disambiguation via E-series check | 🔵 **active** | Medium (2-8h) |
| 3 | ~~[TASK-053](closed/task-053-resistor-uniformity-check.md)~~ | ~~V1 — uniformity check (strict, every deviation flagged)~~ | 🟢 closed | Small (&lt;2h) |
| 4 | ~~[TASK-054](closed/task-054-resistor-extra-packaging.md)~~ | ~~V1 — package as partsledger[resistor-reader] extra with CLI entry-point~~ | 🟢 closed | Small (&lt;2h) |

## EPIC-009: IDEA-012 integration-pass follow-ups

[↑ back to top](#index)

**Status:** 🔵 **active** — ░░░░░░░░░░ 0/2 (0%)

```mermaid
graph TD
    TASK_057["TASK-057"]:::paused
    TASK_058["TASK-058"]:::paused
    TASK_043["TASK-043"]:::activeExt
    TASK_054["TASK-054"]:::closedExt
    TASK_043 --> TASK_057
    TASK_054 --> TASK_058
    click TASK_057 "open/task-057-pipeline-test-fixture-corpus.md"
    click TASK_058 "open/task-058-resistor-output-parser-test.md"
    click TASK_043 "active/task-043-recognition-pipeline-glue.md"
    click TASK_054 "closed/task-054-resistor-extra-packaging.md"
    classDef open    fill:#FAFAFA,stroke:#555,stroke-width:8px,color:#000
    classDef active  fill:#FAFAFA,stroke:#1A6FA8,stroke-width:8px,color:#000
    classDef closed  fill:#FAFAFA,stroke:#3F8B53,stroke-width:8px,color:#000
    classDef paused  fill:#FAFAFA,stroke:#B07810,stroke-width:8px,color:#000
    classDef openExt    fill:#000,stroke:#888,stroke-width:8px,color:#FFF
    classDef activeExt  fill:#000,stroke:#3FA9F5,stroke-width:8px,color:#FFF
    classDef closedExt  fill:#000,stroke:#7CC68A,stroke-width:8px,color:#FFF
    classDef pausedExt  fill:#000,stroke:#F0B030,stroke-width:8px,color:#FFF
```

| Order | ID | Title | Status | Effort |
|-------|----|-------|--------|--------|
| 1 | [TASK-057](paused/task-057-pipeline-test-fixture-corpus.md) | Build pipeline test-fixture corpus under tests/fixtures/ | 🟡 **paused** | Medium (2-8h) |
| 2 | [TASK-058](paused/task-058-resistor-output-parser-test.md) | One-line parser test walking IDEA-011 V1 output through /inventory-add's parser | 🟡 **paused** | Small (&lt;2h) |
