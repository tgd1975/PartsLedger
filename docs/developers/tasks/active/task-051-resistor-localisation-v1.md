...
> **Implementation note (2026-06-21).** `src/partsledger/resistor_reader/localise.py`
> implements `locate_resistors(image) -> list[Candidate]` (HSV body gating
> for beige/blue bodies → morphological close to bridge band gaps → contour
>
> + `minAreaRect`, ranked by a rectangularity × edge-clipping confidence).
> Validated on a **synthetic** fixture set generated by
> `tests/fixtures/resistor-reader/generate.py` (single / three-in-a-row /
> busy-background / partial-out-of-frame), 5 tests in
> `tests/resistor_reader/test_localise.py`. Stays **active** because its
> `human-in-loop: Support` definition-of-done requires validation on **real
> resistor photos** — the synthetic fixtures prove the algorithm in
> principle but can't stand in for real lighting/glare/curvature.
