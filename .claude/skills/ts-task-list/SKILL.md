...
> auto-maintained by `scripts/housekeep.py` and is always up to date.
...
(show open + active + paused). Active tasks are visually distinguished —
render their title in **bold** and include an `active` badge in the
status column. Paused tasks (when shown) get a `paused` badge.
...
After all sections, print the total count of shown tasks (un-epic'd +
all epics). When `--paused` or `--all` is set, the count and an
explicit "(including N paused)" suffix should be shown.
